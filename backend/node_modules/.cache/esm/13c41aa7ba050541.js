"use strict";;let registerData,registerFinalCode,resetPasswordcode,nodemailer,express,bodyParser;_ba0‍.x([["register",()=>route]]);_ba0‍.w("../../serviceLayer/code_register.mjs",[["registerData",["registerData"],function(v){registerData=v}],["registerFinalCode",["registerFinalCode"],function(v){registerFinalCode=v}],["resetPasswordcode",["resetPasswordcode"],function(v){resetPasswordcode=v}]]);_ba0‍.w("nodemailer",[["*",null,function(v){nodemailer=v}]]);_ba0‍.w("express",[["default",["express"],function(v){express=v}]]);_ba0‍.w("body-parser",[["default",["bodyParser"],function(v){bodyParser=v}]]);
...
route.post("/", async (req, res, next) => {
    ...
    smtpTransport.sendMail(mailOptions, function (error, response) {
        ...
        if (error) {
            _ba0‍.g.console.log('Erreur sendMail : ' + error + ' \n');
            res.end("error");
        } else {
            res.end("sent");
        }
    ...
    if ((req.protocol + "://" + req.get('host')) == ("http://" + host)) {
        ...
        if (req.query.id == rand) {
            ...
            registerFinalCode(email, status_validation, status_compte).then ( () => {
                res.json({
                    message: 'Your account has been registered ! Click <a href=' + link + '> here </a> to authenticate urself',
                });
            });
...
route.post('/forgotPassword', async (req, res, next) => {
    ...
    smtpTransport.sendMail(mailOptions, function (error, response) {
        ...
        if (error) {
            _ba0‍.g.console.log('Erreur sendMail forgot password: ' + error + ' \n');
            res.end("error");
        } else {
            res.end("sent");
        }
